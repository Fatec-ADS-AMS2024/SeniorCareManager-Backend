...
namespace SeniorCareManager.WebAPI.Data.Builders
{
    public class ProductBuilder
    {
        public static void Build(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Product>().HasOne(p => p.ProductType).WithMany(pt => pt.Products).HasForeignKey(p => p.ProductTypeId).OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<Product>().HasOne(x => x.UnitOfMeasure).WithMany(x => x.Products).HasForeignKey(x => x.UnitOfMeasureId).OnDelete(DeleteBehavior.Restrict);

            modelBuilder.Entity<Product>().HasMany(p => p.ProductSuppliers).WithMany(ps => ps.ProductSuppliers).UsingEntity(x => x.ToTable("product_supplier"));
